...
Codedir = [Workdir '/repeat_code/Single_parcellation_FC_Variability/step_2_Single_parcelation/S2_kong_2022_HCPA'];
if ~exist(Codedir)
    mkdir(Codedir);
end
cd(Codedir);
...
SubjectsFolder = '/GPFS/cuizaixu_lab_permanent/wuguowei/processing';
idFile = '/GPFS/cuizaixu_lab_permanent/wuguowei/python_code/code/repeat_code/motion_data/HCP_Adults_motion_valid.csv';
...
%%% output sturctures
ProjectFolder = [Workdir '/project/SingleParcellation_Kong_HCPA'];
...
IndiParcFolder = [ProjectFolder '/individual_parcellation_400'];
...
total_sess = '2';
sub_all = readtable(idFile);
total_subs = size(sub_all.subname,1);
