...
Codedir = [Workdir '/repeat_code/Single_parcellation_FC_Variability/step_2_Single_parcelation/S2_kong_2022_HCPD'];
if ~exist(Codedir)
    mkdir(Codedir);
end
cd(Codedir);
...
SubjectsFolder = '/GPFS/cuizaixu_lab_permanent/Public_Data/HCPD/processed_data/';
idFile = '/GPFS/cuizaixu_lab_permanent/wuguowei/python_code/code/repeat_code/motion_data/task_and_rest_motion_valid_sub.csv';

TemplateFolder = [Workdir '/data'];
yeoAtlas = [TemplateFolder '/YeoAtlas/RSN-networks.32k_fs_LR.dlabel.nii'];

%%% output sturctures
ProjectFolder = [Workdir '/project/SingleParcellation_Kong_HCPD_rp2'];
...
total_subs = size(all_sub.subject,1);
